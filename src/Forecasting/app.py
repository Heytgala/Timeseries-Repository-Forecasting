...
@app.route('/api/statrelease', methods=['POST'])
def statrelease():
    body = request.get_json()
    release_req_response = body["pull"]
    repo_name = body["repo"]
    type = body["type"]
    ...
    df = pd.DataFrame(release_req_response)
    df1 = df.groupby(['created_at'], as_index = False).count()
    dataFrame = df1[['created_at','release_number']]
    dataFrame.columns = ['ds', 'y']
    dataFrame.set_index('y')
    period = len(dataFrame) // 2
    predict = sm.tsa.seasonal_decompose(dataFrame.index, period=period)
    figure = predict.plot()
    figure.set_size_inches(12,7)
    plt.title("Observations plot of release requests")
    figure.get_figure().savefig(LOCAL_IMAGE_PATH + OBSERVATION_IMAGE_NAME)               #observation image
    model = sm.tsa.ARIMA(dataFrame['y'].iloc[1:], order = (1, 0, 0))
    results = model.fit()
    dataFrame['forecast'] = results.fittedvalues
    fig = dataFrame[['y', 'forecast']].plot(figsize=(12,7))
    plt.title("Timeseries forecasting of release requests")
    fig.get_figure().savefig(LOCAL_IMAGE_PATH + FORECAST_IMAGE_NAME)
...
@app.route('/api/statcontributors', methods=['POST'])
def statcontributors():
    body = request.get_json()
    contributor_req_response = body["pull"]
    repo_name = body["repo"]
    type = body["type"]
    ...
    df = pd.DataFrame(contributor_req_response)
    df1 = df.groupby(['created_at'], as_index = False).count()
    dataFrame = df1[['created_at','contributor_number']]
    dataFrame.columns = ['ds', 'y']
    dataFrame.set_index('y')
    period = len(dataFrame) // 2
    predict = sm.tsa.seasonal_decompose(dataFrame.index, period=period)
    figure = predict.plot()
    figure.set_size_inches(12,7)
    plt.title("Observations plot of Contributors")
    figure.get_figure().savefig(LOCAL_IMAGE_PATH + OBSERVATION_IMAGE_NAME)               #observation image
    model = sm.tsa.ARIMA(dataFrame['y'].iloc[1:], order = (1, 0, 0))
    results = model.fit()
    dataFrame['forecast'] = results.fittedvalues
    fig = dataFrame[['y', 'forecast']].plot(figsize=(12,7))
    plt.title("Timeseries forecasting of Contributors")
    fig.get_figure().savefig(LOCAL_IMAGE_PATH + FORECAST_IMAGE_NAME)
...
@app.route('/api/statbranches', methods=['POST'])
def statbranches():
    body = request.get_json()
    branch_req_response = body["pull"]
    repo_name = body["repo"]
    type = body["type"]
    ...
    df = pd.DataFrame(branch_req_response)
    df1 = df.groupby(['created_at'], as_index = False).count()
    dataFrame = df1[['created_at','branch_number']]
    dataFrame.columns = ['ds', 'y']
    dataFrame.set_index('y')
    period = len(dataFrame) // 2
    predict = sm.tsa.seasonal_decompose(dataFrame.index, period=period)
    figure = predict.plot()
    figure.set_size_inches(12,7)
    plt.title("Observations plot of Branches")
    figure.get_figure().savefig(LOCAL_IMAGE_PATH + OBSERVATION_IMAGE_NAME)               #observation image
    model = sm.tsa.ARIMA(dataFrame['y'].iloc[1:], order = (1, 0, 0))
    results = model.fit()
    dataFrame['forecast'] = results.fittedvalues
    fig = dataFrame[['y', 'forecast']].plot(figsize=(12,7))
    plt.title("Timeseries forecasting of Branches")
    fig.get_figure().savefig(LOCAL_IMAGE_PATH + FORECAST_IMAGE_NAME)
...
def fbprophetis():
    ...
    df = pd.DataFrame(issues)
    #print("FB Prophet DataFrame",df)
    df['created_at'] = pd.to_datetime(df['created_at'])
    print("DF datatypes",df.dtypes)
    #df1 = df.groupby(['created_at'], as_index = False).count()
    dataFrame = df[['created_at','issue_number']]
    print("DataFram datatypes",dataFrame.dtypes)
    dataFrame = dataFrame.rename(columns={'created_at': 'ds', 'issue_number': 'y'})
    print("FB Prophet DataFrame",dataFrame)
    #dataFrame.columns = ['ds', 'y']
    #print("FB updated DataFrame", dataFrame)
...
@app.route('/api/fbprophetrelease', methods=['POST'])
def fbprophetrelease():
    body = request.get_json()
    release_req_response = body["pull"]
    repo_name = body["repo"]
    type = body["type"]
...
@app.route('/api/fpcontributors', methods=['POST'])
def fpcontributors():
    body = request.get_json()
    contributor_req_response = body["pull"]
    repo_name = body["repo"]
...
@app.route('/api/fpbranches', methods=['POST'])
def fpbranches():
    body = request.get_json()
    branch_req_response = body["pull"]
    repo_name = body["repo"]
...
def releases():
    ...
    data_frame = pd.DataFrame(release_req_response)
    print(data_frame)
    df1 = data_frame.groupby(["created_at"], as_index=False).count()
    df = df1[["created_at", 'release_number']]
    df.columns = ['ds', 'y']
    print(df)
    df['ds'] = pd.to_datetime(df['ds'])
    array = df.to_numpy()
   ...
   # X_test, Y_test = create_dataset(test, look_back)
    ...
    plt.plot(history.history['loss'], label='Train Loss')
    plt.plot(history.history['val_loss'], label='Test Loss')
    plt.title('Model Loss For ' + "Releases")
    plt.ylabel('Loss')
    plt.xlabel('Epochs')
    plt.legend(loc='upper right')
    # Save the figure in /static/images folder
    ...
    y_pred, 'r', label="prediction")
    axs.legend()
    axs.set_title('LSTM Generated Data For ' + "Releases")
    axs.set_xlabel('Time Steps')
    ...
    axs.xaxis.set_major_formatter(mdates.AutoDateFormatter(locator))
    axs.legend()
    axs.set_title('All Releases Data')
    axs.set_xlabel('Date')
    axs.set_ylabel('Releases')
    # Save the figure in /static/images folder
    plt.savefig(LOCAL_IMAGE_PATH + ALL_ISSUES_DATA_IMAGE_NAME)
...
def contributors():
    ...
    data_frame = pd.DataFrame(contributor_req_response)
    print(data_frame)
    df1 = data_frame.groupby(["created_at"], as_index=False).count()
    df = df1[["created_at", 'contributor_number']]
    df.columns = ['ds', 'y']
    print(df)
    df['ds'] = pd.to_datetime(df['ds'])
    array = df.to_numpy()
   ...
   # X_test, Y_test = create_dataset(test, look_back)
    ...
    plt.plot(history.history['loss'], label='Train Loss')
    plt.plot(history.history['val_loss'], label='Test Loss')
    plt.title('Model Loss For ' + "Contributors")
    plt.ylabel('Loss')
    plt.xlabel('Epochs')
    plt.legend(loc='upper right')
    # Save the figure in /static/images folder
    ...
    y_pred, 'r', label="prediction")
    axs.legend()
    axs.set_title('LSTM Generated Data For ' + "Contributors")
    axs.set_xlabel('Time Steps')
    ...
    axs.xaxis.set_major_formatter(mdates.AutoDateFormatter(locator))
    axs.legend()
    axs.set_title('All Contributors Data')
    axs.set_xlabel('Date')
    axs.set_ylabel('Contributors')
    # Save the figure in /static/images folder
    plt.savefig(LOCAL_IMAGE_PATH + ALL_ISSUES_DATA_IMAGE_NAME)
...
def branches():
    ...
    data_frame = pd.DataFrame(branch_req_response)
    print(data_frame)
    df1 = data_frame.groupby(["created_at"], as_index=False).count()
    df = df1[["created_at", 'branch_number']]
    df.columns = ['ds', 'y']
    print(df)
    df['ds'] = pd.to_datetime(df['ds'])
    array = df.to_numpy()
   ...
   # X_test, Y_test = create_dataset(test, look_back)
    ...
    y_pred, 'r', label="prediction")
    axs.legend()
    axs.set_title('LSTM Generated Data For ' + "Branches")
    axs.set_xlabel('Time Steps')
